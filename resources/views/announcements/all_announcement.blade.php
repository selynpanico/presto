<x-layout>
    ...
    <form method="GET"class="d-flex" action="{{route('ricerca.annuncio')}}">
        ...
        <div class="col-12 col-md-6 d-flex"> 
            <a href="{{ route('all.announcements')}}"><img class="m-2" src="/img/icone/presto_icona-all.svg" alt="tutte le categorie" width="10%"></a> 
            @foreach($categories as $category)
            <a href="{{route('category.show', $category)}}"><img class="m-2" src="{{ $category->icon}}" alt="{{ $category->name}}" width="10%"></a> 
            @endforeach
        </div>
        </div>
    </div>
